...
namespace ArchiveLibraryBooksConsoleApplication
{
    ...
    {
        private const string PathToBooksFileZip = "Books.zip";
        ...
        private void FillBooksList()
        {
            ...
            try
            {
                using (var zipToOpen = new FileStream(PathToBooksFileZip, FileMode.Open))
                {
                    using (var archive = new ZipArchive(zipToOpen, ZipArchiveMode.Read))
                    {
                        var archiveEntry = archive.GetEntry(PathToBooksFile);
        ...
        public void SaveCurrentStateOfTheLibrary()
        {
            ...
            try
            {
                using (var zipToOpen = new FileStream(PathToBooksFileZip, FileMode.Open))
                {
                    ...
                    {
                        var entry = archive.GetEntry(PathToBooksFile);
                        entry.Delete();
                        File.WriteAllLines(PathToBooksFile, booksSerializationArray);                     
                        archive.CreateEntryFromFile(PathToBooksFile, "Books.txt");
                    }
